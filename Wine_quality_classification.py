...
class WineTypeModel:
    ...
    def _plot_roc_curve(self, model):
        ...
        for i in class_name:
            plt.plot(
                fpr[i], tpr[i], linewidth = 2, label = f"class {i}"
            )
        ...
        plt.title(f"{self.name} ROC curve_Quality")
        plt.legend(loc = 'lower right')
        plt.savefig(f"result/{self.name} ROC curve_Quality", dpi = 300)
        # plt.show()
        plt.close()
    ...
    def _plot_decision_region(self, model):
        ...
        plt.title(f"{self.name} Decision Region_Quality")
        plt.savefig(f"result/{self.name} Decision Region_Quality", dpi = 300)
        # plt.show()
        plt.close()
...
def main():
    ...
    model_tune_result.to_csv("result/Result_Quality.csv", index = False)
